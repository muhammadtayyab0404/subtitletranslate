...
# Keep your original explainer model unchanged
EXPLAINER_MODEL_NAME = "Qwen/Qwen2.5-3B-Instruct"

# Keep your original sampling behavior unchanged
AI_DO_SAMPLE = True
AI_TEMPERATURE = 0.7
AI_TOP_P = 0.9
AI_TOP_K = 40
...
# Optional memory optimization only; does not change outputs
...
def _generate_with_explainer(prompt, max_new_tokens=120, keep_newlines=False):
    # Keep uncached when sampling is enabled, so behavior remains unchanged
    ...
    return _cleanup_generated_text(text, keep_newlines=keep_newlines)
...
def _word_meanings_cached(text, translated_text):
...
def word_meanings(text, source_lang, target_lang, translated_text=None):
    ...
    # cache depends only on text + translated_text; same output as before
    return list(_word_meanings_cached(str(text), str(translated_text)))
...
# =========================
# Shared analysis pivot
# =========================
def _get_english_pivot(text, source_tag, target_tag, translated_text=None):
    if source_tag == "eng_Latn":
        return text
    if target_tag == "eng_Latn" and translated_text:
        return translated_text
    return _generate_translation(text, source_tag, "eng_Latn", max_new_tokens=120)
...
def _generate_explanation_english(english_text):
    ...
    explanation = _generate_with_explainer(prompt, max_new_tokens=80, keep_newlines=False)
    ...
    if _looks_garbled(explanation) or too_close or explanation.strip().endswith("?"):
        ...
        explanation = _generate_with_explainer(retry_prompt, max_new_tokens=90, keep_newlines=False)
...
def _sentence_descriptions_from_english(english_input, source_tag, target_tag):
...
def sentence_descriptions(text, source_lang, target_lang, translated_text=None):
    ...
    english_input = _get_english_pivot(
        text=text,
    ...
    return _sentence_descriptions_from_english(english_input, source_tag, target_tag)
...
def _generate_ai_grammar_notes(english_text, tense, structure, subject, verb, obj):
    ...
    notes_text = _generate_with_explainer(prompt, max_new_tokens=120, keep_newlines=True)
...
def _sentence_grammar_analysis_from_english(english_input, source_tag, target_tag):
...
def sentence_grammar_analysis(text, source_lang, target_lang, translated_text=None):
    ...
    return _sentence_grammar_analysis_from_english(english_input, source_tag, target_tag)
...
def translate(text, source_lang, target_lang, max_new_tokens=200):
    ...
    # Compute once, print once (same printed output, less repeated work)
    word_items = word_meanings(text, source_tag, target_tag, translated_text=result)
    if not word_items:
        print("No contextual word meanings found.")
    else:
        print("Context-aware word meanings:")
        for i, (src, tgt) in enumerate(word_items, start=1):
            print(f"{i}. {src} -> {tgt}")
        print()
    ...
    descriptions = _sentence_descriptions_from_english(english_input, source_tag, target_tag)
    print("Sentence description:")
    print(f"- English explanation: {descriptions['english_explanation']}")
    print(f"- Source language ({source_tag}): {descriptions['source_description']}")
    print(f"- Target language ({target_tag}): {descriptions['target_description']}")
    ...
    grammar = _sentence_grammar_analysis_from_english(english_input, source_tag, target_tag)
    print("Grammar analysis:")
    print(f"- English:\n{grammar['english_grammar']}\n")
    print(f"- Source language ({source_tag}):\n{grammar['source_grammar']}\n")
    print(f"- Target language ({target_tag}):\n{grammar['target_grammar']}\n")
