...
# If your machine is strong:
#   "Qwen/Qwen2.5-7B-Instruct"
# If your machine is weaker:
#   "Qwen/Qwen2.5-1.5B-Instruct"
#   "Qwen/Qwen2.5-0.5B-Instruct"
EXPLAINER_MODEL_NAME = "Qwen/Qwen2.5-3B-Instruct"

# AI generation settings (used only for explainer model)
AI_DO_SAMPLE = True
AI_TEMPERATURE = 0.7
AI_TOP_P = 0.9
AI_TOP_K = 40
...
if explainer_device == "cuda":
    ...
    explainer_model = AutoModelForCausalLM.from_pretrained(EXPLAINER_MODEL_NAME)
...
def _build_qwen_prompt(user_prompt: str):
    ...
    if hasattr(explainer_tokenizer, "apply_chat_template"):
        return explainer_tokenizer.apply_chat_template(
            messages,
            tokenize=False,
...
def _generate_with_explainer(prompt, max_new_tokens=120, keep_newlines=False):
    ...
    inputs = explainer_tokenizer(rendered_prompt, return_tensors="pt").to(explainer_device)
    ...
    with torch.inference_mode():
        outputs = explainer_model.generate(
            **inputs,
            ...
            temperature=AI_TEMPERATURE if AI_DO_SAMPLE else None,
            top_p=AI_TOP_P if AI_DO_SAMPLE else None,
            top_k=AI_TOP_K if AI_DO_SAMPLE else None,
            repetition_penalty=1.08,
            pad_token_id=explainer_tokenizer.pad_token_id,
            eos_token_id=explainer_tokenizer.eos_token_id,
        )

    new_tokens = outputs[0, inputs["input_ids"].shape[1]:]
    text = explainer_tokenizer.decode(new_tokens, skip_special_tokens=True)
